...
import 'package:gap/gap.dart'; // Import agora será utilizado
...
class _AuditScreenState extends ConsumerState<AuditScreen> with SingleTickerProviderStateMixin {
  ...
  // === IMPORTAÇÃO EXCEL COM CORREÇÃO DE NULL SAFETY ===
      ...
      if (sheet != null) {
        if (isAsset) {
          ...
          for (var i = 1; i < sheet.rows.length; i++) {
            ...
            // Correção: Garantindo String não nula para campos 'late'
            items.add(AssetItem()
    ...
    showModalBottomSheet(
      ...
      builder: (context) => SafeArea(
        child: Column(
          ...
          children: [
            const Gap(10), // Uso do Gap para evitar warning de unused import
            ListTile(
              leading: const Icon(Icons.agriculture, color: Colors.green),
              title: const Text("Importar Bens (Excel)"),
              subtitle: const Text("Colunas: Descrição, Série, Placa, Categoria..."),
              onTap: () { Navigator.pop(context); _importExcel(true); },
            ),
            ListTile(
              leading: const Icon(Icons.landscape, color: Colors.brown),
              title: const Text("Importar Fazendas (Excel)"),
              subtitle: const Text("Colunas: Nome, Matrícula, Cidade, UF, Lat, Long"),
              onTap: () { Navigator.pop(context); _importExcel(false); },
            ),
            const Gap(20),
          ],
        ),
          ...
          if (_isLoading)
            ...
            IconButton(
              ...
              onPressed: () async {
                ...
                final props = await repo.getPropertiesSync(widget.project.id);
...
class _AssetsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return assetsAsync.when(
      ...
      data: (assets) {
        if (assets.isEmpty) return const Center(child: Text("Nenhum bem. Use o botão de importação."));
          ...
          itemBuilder: (context, index) {
            ...
            return ListTile(
              leading: CircleAvatar(
                ...
                child: Icon(_getStatusIcon(item.status), color: Colors.white, size: 18),
              ),
  ...
  IconData _getStatusIcon(AuditStatus status) {
    switch (status) {
      case AuditStatus.found: return Icons.check;
      case AuditStatus.notFound: return Icons.search_off;
      case AuditStatus.seized: return Icons.gavel;
...
class _PropertiesTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return propsAsync.when(
      ...
      error: (e, s) => Center(child: Text("Erro: $e")),
      data: (props) {
        if (props.isEmpty) return const Center(child: Text("Nenhuma fazenda. Use o botão de importação."));
        return Column(
          children: [
            Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text("Processar Fotos de Drone"),
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text("${item.matricula} - ${item.city}"),
                    trailing: IconButton(
